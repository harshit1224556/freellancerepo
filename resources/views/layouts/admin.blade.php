...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard - Growing India')</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        :root{--accent:#ffffff;--dark:#050505;--card:#111111;--border:rgba(255,255,255,0.07);--muted:#888888}
        ...
        .logout-btn{width:100%;padding:.7rem;background:#ffffff;border:1px solid #ffffff;color:#000000;border-radius:10px;cursor:pointer;font-family:'Inter',sans-serif;font-size:.9rem;font-weight:600;transition:all .3s;text-align:center}
        ...
        .btn-primary{background:#fff;color:#000;padding:.7rem 1.5rem;border-radius:12px;border:none;cursor:pointer;font-family:'Inter',sans-serif;font-weight:600;font-size:.9rem;transition:all .3s;text-decoration:none;display:inline-block;}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('admin.dashboard') }}" class="nav-item {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}"><span class="nav-icon">📊</span> Overview</a>
        <a href="{{ route('admin.applicants') }}" class="nav-item {{ request()->routeIs('admin.applicants') ? 'active' : '' }}"><span class="nav-icon">👥</span> Applicants</a>
        <a href="{{ route('admin.campaigns') }}" class="nav-item {{ request()->routeIs('admin.campaigns') ? 'active' : '' }}"><span class="nav-icon">📣</span> Campaigns</a>
        <a href="{{ route('admin.companies') }}" class="nav-item {{ request()->routeIs('admin.companies') ? 'active' : '' }}"><span class="nav-icon">🏢</span> Companies</a>
        <div class="nav-label" style="margin-top:2rem">System</div>
        <a href="{{ route('admin.settings') }}" class="nav-item {{ request()->routeIs('admin.settings') ? 'active' : '' }}"><span class="nav-icon">⚙️</span> Settings</a>
    </nav>
    <div class="sidebar-footer">
        <div class="user-info">
            <div class="user-avatar">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</div>
            <div><div class="user-name">{{ auth()->user()->name }}</div><div class="user-role">Administrator</div></div>
        </div>
